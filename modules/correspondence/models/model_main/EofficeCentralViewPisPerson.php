...
class EofficeCentralViewPisPerson extends \yii\db\ActiveRecord
{
    ...
    {
        return [
            [['person_id', 'person_type', 'person_religion_id', 'id'], 'integer'],
            [['person_card_id', 'person_name', 'person_surname', 'person_email', 'person_type',
                'person_religion_id'], 'required'],
            [['person_birthdate', 'person_start_date', 'person_contract_date', 'person_expire_date',
                'person_confirmed_date', 'person_pass_probation_date', 'person_retire_date',
                'person_decommission_date'], 'safe'],
            [['person_salary'], 'number'],
            [['person_card_id', 'person_mobile'], 'string', 'max' => 20],
            [['person_name', 'person_name_eng', 'person_surname', 'person_surname_eng', 'PREFIXNAME',
                'PREFIXABB', 'person_gender', 'person_operate_status', 'person_official_age',
                'person_account_hold', 'person_current_work_place', 'person_person_type',
                'person_position_type', 'person_administer_position', 'person_salary_position',
                'person_pension', 'person_pension_withdraw', 'person_talent', 'academic_positions_abb_thai',
                'academic_positions'], 'string', 'max' => 50],
            [['person_current_address', 'person_email', 'major_name_eng', 'major_code', 'NATIONNAME',
                'person_position_staff', 'DEPARTMENTNAME'], 'string', 'max' => 100],
            ...
            [['person_marital_status', 'person_group_blood', 'person_underlying_disease', 'person_website',
                'person_line', 'person_facbook'], 'string', 'max' => 45],
        ];
    }
    ...
    public function getBoardDirector()
    {
        return $this->hasOne(EofficeCentralViewPisBoardOfDirectors::className(),
            ['person_id' => 'person_id']);
    }
    public function getMajor()
    {
        return $this->hasOne(EofficeCentralViewPisMajor::className(),
            ['code' => 'major_code']);
    }
    public function getViewUser()
    {
        return $this->hasOne(EofficeCentralViewPisUser::className(),
            ['id' => 'id']);
    }
}
