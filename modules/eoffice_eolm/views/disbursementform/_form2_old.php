...
<div class="eolm-disbursementform-form">
    ...
    <div class="row">
        <div class="col-md-1 col-md-offset-8 col-sm-1 col-sm-offset-8">
            <?= $form->field($model, 'eolm_dis_date_count')->textInput(['maxlength' => true,'id'=>'calDate'])->label(controllers::t( 'label', 'Amount(day)')) ?>

        </div>
        <div class="col-md-2 col-sm-2">
            <?= $form->field($model, 'eolm_dis_time')->textInput(['maxlength' => true,'id'=>'calTime'])->label(controllers::t( 'label', 'As a (Hours)')) ?>
    ...
    <div class="row">
        ...
        <?php
        use yii\db\Query;
        $connection = \Yii::$app->db_eolm;
        $rate=0;

        $c  = $connection->createCommand('SELECT * FROM eolm_approvalform_has_personal 
                LEFT OUTER JOIN eoffice_central.person ON eoffice_central.person.person_id=eolm_approvalform_has_personal.person_id
                LEFT OUTER JOIN eoffice_central.academic_positions ON eoffice_central.academic_positions.academic_positions_id=eoffice_central.person.academic_positions_id
                LEFT OUTER JOIN eolm_rate_cost ON eolm_rate_cost.academic_positions_id=eoffice_central.person.academic_positions_id
                WHERE eolm_app_id ='.$model->eolm_app_id);
        $cost =  $c->queryAll();
        if(!empty($cost)) {
            echo '<div class="col-md-7 col-sm-7" style="border: 2px solid pink;border-radius: 5px;">';
            echo '<div class="col-md-8 col-sm-8 text-center info"><label>'.controllers::t( 'label', 'Allowance of').' </label></div>
                    <div class="col-md-3 col-sm-3 text-right"><label>'.controllers::t( 'label', 'Once per day (baht)').' </label></div>';
            //$c=0;
           // $one=1;
            foreach ($cost as &$value) {
               // $person = EofficeMainPerson::find()->where(['person_id'=>$value['person_id']])->one();
                echo '<div class="col-md-9 col-sm-9">'.$value['academic_positions_abb_thai'].' '.$value['person_name'].' '.$value['person_surname'].
                     '</div><div class="col-md-2 col-sm-2 divAllow"><span>'.$value['eolm_pos_allowance_rate'].'</span></div>';
                $rate= $rate +$value['eolm_pos_allowance_rate'];
               // $c=$c + $one;
            }
            echo '</div >';
            echo '<div class="col-md-1 col-md-offset-4 col-sm-1 col-sm-offset-4">'.$form->field($model, 'eolm_dis_allowance_day')->textInput(['maxlength' => 3,'id'=>'allowDate'])->label(controllers::t( 'label', 'Number (day)')).'</div>';
            echo '<div class="col-md-2 col-sm-2">'.
                    $form->field($model, 'eolm_dis_allowance_cost')->textInput(['maxlength' => true,'id'=>'allow'])->label(controllers::t( 'label', 'Amount')) .'
    ...
    <div class="row">
        ...
        if(!empty($cost)) {
            echo '<div class="col-md-7 col-sm-7" style="border: 2px solid pink;border-radius: 5px;">';
            echo '<div class="col-md-8 col-sm-8 text-center info"><label>  '.controllers::t( 'label', 'Rent of').'</label></div><div class="col-md-3 col-sm-3 text-right"><label> '.controllers::t( 'label', 'Once per day (baht)').' </label></div>';

            foreach ($cost as &$value) {
                echo '<div class="col-md-9 col-sm-9">'.$value['academic_positions_abb_thai'].' '.$value['person_name'].' '.$value['person_surname'].
                    '</div>
                    <div class="col-md-2 col-sm-2 room " style="display: none;">
                    '.$value['eolm_pos_singlebed_rate'].' 
                    </div>
                    <div class="col-md-2 col-sm-2 room2" style="display: none;">
                    '.$value['eolm_pos_twinbeds_rate'] .'
                    </div>
                    <div class="col-md-2 col-sm-2 room3" style="display: none;">
                    '.$value['eolm_pos_singlebed_rate2'].' 
                    </div>
                    <div class="col-md-2 col-sm-2 room4" style="display: none;">
                    '.$value['eolm_pos_twinbeds_rate2'].'
                    </div>';
                $room= $room +$value['eolm_pos_singlebed_rate'];
                $room2= $room2 +$value['eolm_pos_twinbeds_rate'];
                $room3= $room3 +$value['eolm_pos_singlebed_rate2'];
                $room4= $room4 +$value['eolm_pos_twinbeds_rate2'];
            }
            echo '</div >
             <div class="col-md-4 col-sm-4" id="element">'.$form->field($model, 'eolm_dis_pay_by')->radioList([1=>controllers::t( 'label', 'Flat rate'),2=>controllers::t( 'label', 'Rate of receipts')])->label(false).'
             '.$form->field($model, 'eolm_dis_room_type')->radioList([3=>controllers::t( 'label', 'Single bed'),4=>controllers::t( 'label', 'Double bed')])->label(false).'</div>';
            echo '<div class="col-md-1 col-sm-1">'.$form->field($model, 'eolm_dis_hotal_day')->textInput(['maxlength' => 3,'id'=>'hoDate'])->label(controllers::t( 'label', 'Number (day)')).'</div>';
            echo '<div class="col-md-2 col-sm-2">'.$form->field($model, 'eolm_dis_hotal_cost')->textInput(['maxlength' => true,'id'=>'hotel'])->label(controllers::t( 'label', 'Amount')) .'</div>';
            echo '</div>';
        }else{
            echo '<div class="col-md-2 col-sm-2" id="element">
             <br>'.$form->field($model, 'eolm_dis_pay_by')->radioList(array(1=>controllers::t( 'label', 'Flat rate'),2=>controllers::t( 'label', 'Rate of receipts'))).'
            
            </div>
            <div class="col-md-2 col-sm-2">
                <br>'.$form->field($model, 'eolm_dis_room_type')->radioList(array(1=>controllers::t( 'label', 'Single bed'),2=>controllers::t( 'label', 'Double bed'))).'
                
            </div>';
            echo '<div class="col-md-1 col-sm-1">'.$form->field($model, 'eolm_dis_hotal_day')->textInput(['maxlength' => 3,'id'=>'hoDate'])->label(controllers::t( 'label', 'Number (day)')).'</div>';
            echo '<div class="col-md-2 col-sm-2">'.
                    $form->field($model, 'eolm_dis_hotal_cost')->textInput(['maxlength' => true,'id'=>'hotel'])->label(controllers::t( 'label', 'Amount')) .'
        ...
        <div>
            <?= FileInput::widget([
                ...
                'pluginOptions' => [
                    ...
                    'uploadUrl' => Url::to(['/eoffice_eolm/disbursementform/upload-ajax']),
                    'uploadExtraData' => [
                        'ref' => $model->ref,
                    ],
                    'maxFileCount' => 100
...
<script type="text/javascript">
    function myDate(){
        ...
        document.getElementById("allowDate").value = da;
        ...
        if (isNaN(ho)) ho = 0;
        document.getElementById("hoDate").value = ho;
        if($('.room').is(':visible')) {
