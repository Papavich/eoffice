            ...
            [
                ...
                'buttons'=>[
                    ...
                    'loan' => function ($url, $model,$key) {
                        ...
                        if(empty($lo)){
                            return Html::a('<i class="	fa fa-plus"></i> '.controllers::t( 'menu','Create a Loan'), '@web/eoffice_eolm/loancontractin/create?id='.$model->eolm_app_id, ['class'=>'btn btn-block btn-social btn-success'],[
                                'title' => Yii::t('app', 'ทำสัญญายืมเงิน')
                            ]);
                        }else{
                            $mo = EolmDisbursementform::find()->where('eolm_app_id='.$model->eolm_app_id)->one();
                            if(empty($mo)){
                                return Html::a('<i class="	fa fa-plus"></i> '.controllers::t( 'menu','Create a Disbursement form'), '@web/eoffice_eolm/disbursementform/create?id='.$model->eolm_app_id, ['class'=>'btn btn-block btn-social btn-success'],[
                                    'title' => Yii::t('app', 'ทำใบเบิกจ่าย')
                                ]);
                            }else{
                                return Html::a('<i class="fa fa-pencil-square-o"></i> '.controllers::t( 'menu','Update a Disbursement form'), '@web/eoffice_eolm/disbursementform/update?id='.$model->eolm_app_id, ['class'=>'btn btn-block btn-social btn-warning'],[
