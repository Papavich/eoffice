...
use app\modules\pfc\models\PersonnelFile;
use app\modules\pfc\models\ViewSubject;
use app\modules\pfc\models\ViewSubjectOpen;
...
class PersonnelController extends Controller
{
    ...
    public  function  actionPersonnel_type(){
        ...
        return $this->render('personnel_type', [
            'test_id' => Yii::$app->user->identity->username,
        ]);
    }
    ...
    public  function  actionPersonnel_process_add(){
        $process_gantt = ProcessGantt::find()->where("process_gantt_type_id like :b", [":b" => $_REQUEST['process_gantt_type_id']])->orderBy(['process_gantt_no'=>SORT_ASC])->all();
        $typename = null;
        $count_no = 0;
        ...
        $process_gantt_id_add = 'process_gantt_'.$typename.'_'.($count_no+1);
        $process_gantt_no_add = Yii::$app->request->post('process_no_form');
        $process_gantt_topic_add = Yii::$app->request->post('process_topic_form');
        $process_gantt_detail_add = Yii::$app->request->post('process_detail_form');
        $process_gantt_score_add = 0;
        $process_gantt_type_code_add = Yii::$app->request->post('process_gantt_type_id');
        $process_gantt_date_start_add = Yii::$app->request->post('process_date_start_form');
        $process_gantt_date_end_add = Yii::$app->request->post('process_date_end_form');



        $process_add = New ProcessGantt();
        $process_add->process_gantt_id = $process_gantt_id_add;
        $process_add->process_gantt_no = (int)$process_gantt_no_add;
        $process_add->process_gantt_topic = $process_gantt_topic_add;
        $process_add->process_gantt_detail = $process_gantt_detail_add;
        $process_add->process_gantt_score = $process_gantt_score_add;
        $process_add->process_gantt_type_code = $process_gantt_type_code_add;
        $process_add->process_gantt_date_start = $process_gantt_date_start_add;
        $process_add->process_gantt_date_end = $process_gantt_date_end_add;
        $process_add->save();
...
//        echo 'console.log('. json_encode( $data ) .')';
        ...
        if($personnel == null) {
            ...
            move_uploaded_file($_FILES["upload"]["tmp_name"], "../web/web_pfc/fileupload/staff_file/" . $file_upload_newname);
    ...
    public function actionPersonnel_download($fileupload){
        $session = Yii::$app->session;
        $file_url = '../web/web_pfc/fileupload/staff_file/'.$fileupload;
