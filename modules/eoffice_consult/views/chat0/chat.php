...
use app\models\ConsultChatDetail;
...
<link href="<?= Yii::$app->homeUrl ?>css/chat.css" rel="stylesheet" type="text/css"/> <!-- css ของ chat -->
...
<section id="middle">
    ...
    <ul class="messages">
      ...
      if(isset($chat_msg)) {
          ...
          foreach ($chat_msg as $item):
              if ($item['chat_detail_name'] == (Yii::$app->user->identity->id)) {
                  $type = "right";
              } else {
                  $type = "left";
              }
    ...
    </ul> <!-- ดึงหน้า chat_msg.php มาแสดงในกล่องแชท chat_msg.php คือข้อความของ user -->
    ...
    (function () {
        ...
        $(function () {
            ...
            $('.send_message').click(function (e) {
                $.post('<?= Yii::$app->homeUrl ?>eoffice_consult/chat0/chat_send', {msg: getMessageText()}, function () {});
...
//                $('.messages').load('<?//= Yii::$app->homeUrl ?>//chat/chat_msg');
                ...
                $.get('<?= Yii::$app->homeUrl ?>eoffice_consult/chat0/chat_msg', function (data) {
