        ...
        if(in_array($type, ['docs'])){
            ...
            if(is_array($files)){
                ...
                foreach ($files as $key => $value) {
                    $docs_file .= '<li>'.Html::a($value,['/eoffice_eolmv2/disbursementform/download','id'=>$this->id,'file'=>$key,'file_name'=>$value]).'</li>';
                }
                $docs_file .='</ul>';
            }
        }
            ...
            foreach ($files as $key => $value) {
                if($type=='file'){
                    ...
                    $initial[] = [
                        'caption'=> $value,
                        'width'  => '120px',
                        'url'    => Url::to(['/eoffice_eolmv2/disbursementform/deletefile-ajax','id'=>$this->id,'fileName'=>$key,'field'=>$field]),
                        'key'    => $key
                    ];
                }
                else{
