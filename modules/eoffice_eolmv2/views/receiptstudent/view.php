...
$this->title = \app\modules\eoffice_eolmv2\controllers::t( 'menu','Details for receipt for student');
...
if ($userType==AuthHelper::TYPE_ADMIN){
    $this->params['breadcrumbs'][] = ['label' => \app\modules\eoffice_eolmv2\controllers::t( 'menu','Search approval form'), 'url' => ['index', 'id' => $_GET["eolm_app_id"]]];
}elseif ($userType==AuthHelper::TYPE_TEACHER||$userType == AuthHelper::TYPE_APPROVERS){
    $this->params['breadcrumbs'][] = ['label' => \app\modules\eoffice_eolmv2\controllers::t( 'menu','Search approval form'), 'url' => ['indexaj', 'id' => $_GET["eolm_app_id"]]];
}
...
<div class="eolm-receipt-student-view">
    ...
    <?= DetailView::widget([
        ...
        'attributes' => [
            ...
            [
                'attribute'=> 'person_id',
                'label' =>\app\modules\eoffice_eolmv2\controllers::t('label_appform','Student Code'),
            ]
            ,
            [
                ...
                'value' => function($model) {
                    ...
                    $model = \app\modules\eoffice_eolmv2\models\model_main\EofficeMainViewStudentFull::findBySql($sql)->one();
                    $txt= $model['STUDENTNAME']." ".$model['STUDENTSURNAME'];
                    return $txt;
                }
            ],[
                'attribute'=> 'eolm_rec_std_total',
                'label' =>\app\modules\eoffice_eolmv2\controllers::t('label','Total'),
            ],
            /*'crby',
            'crtime',
            'udby',
    ...
    <div class="form-group text-center">
        ...
        if ($userType==\app\modules\eoffice_eolmv2\components\AuthHelper::TYPE_ADMIN){
            echo Html::a(\app\modules\eoffice_eolmv2\controllers::t('label','Back'), ['index','id'=>$model->eolm_app_id], ['class' => 'btn btn-primary']);
