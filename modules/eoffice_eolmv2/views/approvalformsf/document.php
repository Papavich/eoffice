             ...
             /// 'options'=> ['style'=>'width:250px;'],
              'buttons'=>[
                    ...
                    'disbursementform/excel2' => function($url,$model,$key){
                      ...
                      $items = \app\modules\eoffice_eolmv2\models\EolmDisbursementformDetailsItem::find()->where(['eolm_app_id'=> $model->eolm_app_id,'eolm_dis_detail_bill'=>2,'eolm_dis_type'=>1])->orWhere(['eolm_dis_type'=>2])->groupBy('person_id')
                          ->all();
                        $button='';
                        foreach($items as $item) {
                            $per1 = \app\modules\eoffice_eolmv2\models\model_main\EofficeMainViewPisPerson::find()->where(['person_id'=>$item["person_id"]])->one();
                  ...
                  'disbursementform/download' => function($url,$model,$key){
                        $modeld=\app\modules\eoffice_eolmv2\models\EolmDisbursementform::find()->where(['eolm_app_id'=> $model->eolm_app_id])->one();

                        $file=\app\modules\eoffice_eolmv2\models\Uploads::find()->where(['ref'=>$modeld->ref])->asArray()->all();
                    ...
                    /*'repayform/word' => function($url,$model,$key){
                      ...
                      $loan = \app\modules\eoffice_eolmv2\models\EolmRepay::findAll(['eolm_app_id'=> $model->eolm_app_id]);
