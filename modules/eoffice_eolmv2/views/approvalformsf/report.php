...
//        'จังหวัด'
        ...
        ['attribute'=>function($model) {
            $sql = 'SELECT * FROM eoffice_eolmv2.eolm_loancontract WHERE eoffice_eolmv2.eolm_loancontract.eolm_app_id='.$model->eolm_app_id;
            $model = \app\modules\eoffice_eolmv2\models\EolmLoancontract::findBySql($sql)->one();
            return $model['eolm_loa_total_amout'];
        }, 'label'=>'จำนวนเงิน'],
        ['attribute'=>function($model) {
            $sql = 'SELECT * FROM eoffice_eolmv2.eolm_disbursementform WHERE eoffice_eolmv2.eolm_disbursementform.eolm_app_id='.$model->eolm_app_id;
            $model = \app\modules\eoffice_eolmv2\models\EolmDisbursementform::findBySql($sql)->one();
            return $model['eolm_dis_total'];
        }, 'label'=>'จ่ายจริง'],
        ['attribute'=>function($model) {
            $sql = 'SELECT * FROM eoffice_eolmv2.eolm_repay WHERE eoffice_eolmv2.eolm_repay.eolm_app_id='.$model->eolm_app_id;
            $model = \app\modules\eoffice_eolmv2\models\EolmRepay::findBySql($sql)->one();
    ...
    <?= GridView::widget([
        ...
        'columns' => [
            ...
            [ // แสดงชื่อ
                ...
                'format'=>'html',
                'filter'=>ArrayHelper::map(\app\modules\eoffice_eolmv2\models\EolmType::find()->all(),'eolm_type_id','eolm_type_name'),
