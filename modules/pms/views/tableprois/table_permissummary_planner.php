...
<header id="page-header">
    <h1>ขอจัดโครงการและใช้งบประมาณ</h1>
    <?php
        ...
        <div class="col-md-12 col-lg-12">
            <div class="panel panel-default">
                <div class="panel-heading panel-heading-transparent">
                    <strong>โครงการที่ขอจัดโครงการและใช้งบประมาณ</strong>
                </div>
                    ...
                    <div class="col-md-12" style="margin-top: 20px">
                        <table id="table_compact_summary" class="table table-striped table-bordered">
                            ...
                            foreach ($prosub as $key => $rows){
                                if($rows['status_result']=="รองานนโยบายและแผนอนุมัติ") {
                                    $i = $key + 1;
                                    ?>
                                    <tr>
                                        <td>
                                            <?php
                                            $stg ="".$rows['compact_start_date'];
                                            $date = substr($stg, 0, 10);
                                            echo "จัดครั้งที่ ".$rows['round']." ".$date;
                                            //echo $date;
                                            ?>
                                        </td>
                                        <td class="center">
                                        ...
                                        <td>
                                            <a href="#" data-toggle="modal" class="get_status_summary" data="<?=$rows['prosub_code']."_".$rows['id']?>" data-target="#showstatus">
                                                <?php
                                        ...
                                        <td>
                                                <span>
                                                    <?php
                                                    $comment = \app\modules\pms\models\LogPermisInSystem::find()->where(['pms_project_sub_prosub_code'=>$rows['prosub_code'],'status_process'=>4,'pms_compact_has_prosub_id'=>$rows['id']])->orderBy(['id'=>SORT_DESC])->one();
                                                    if($comment){
                                        ...
                                        <td>
                                            <?php
                                            $data = \app\modules\pms\models\PmsProjectSub::find()->where(['prosub_code'=>$rows['prosub_code']])->one();
                                        ...
                                        <td>
                                            <a><i class=""></i></a>
                                            <?php
                                            echo "<a href=\"../detailprosubresult/detail-planner?id=".$rows['prosub_code']."&id_compact=".$rows['id']."\"><i class=\"glyphicon glyphicon-eye-open\"></i></a> ";
                                            echo " | <a href=\"../permissionsummary/permis-planner?id=".$rows['prosub_code']."&compact=".$rows['id']."\"><i class=\"glyphicon glyphicon-check\"></i></a> ";
                                            ?>
