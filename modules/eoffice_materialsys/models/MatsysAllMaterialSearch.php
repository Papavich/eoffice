 ...
 */
class MatsysAllMaterialSearch extends MatsysMaterial
{
    /**
     * @inheritdoc
     */
    ...
    {
        if(isset($params['search']) && isset($params['type'])){
            if ($params['search'] == ''){
                $query = MatsysMaterial::find()
                    ->joinWith('matsysBillDetails', false, 'INNER JOIN')
                    ->andFilterWhere(['like', 'material_type_id', $params['type']]);
            }else{
                $query = MatsysMaterial::find()
                    ->joinWith('matsysBillDetails', false, 'INNER JOIN')
                    ->andFilterWhere(['like', 'material_type_id', $params['type']])
                    ->andFilterWhere(['like', 'material_name', $params['search']]);
            }
