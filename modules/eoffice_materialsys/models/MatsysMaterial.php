...
class MatsysMaterial extends \yii\db\ActiveRecord
{
    ...
    {
        return [
            [['material_id', 'location_id', 'material_type_id', 'material_unit_name','material_amount_check','material_name','material_detail'], 'required'],
...
//    All Amount Return
    public static function amountAllReturn($material_id){
        $count =  MatsysOrderReturn::find()
            ->where("material_id = :userid ", [':userid' => $material_id])
            ->andWhere('order_return_date_accept')
            ->sum('order_return_amount_use');
        if($count == null){
            return '0';
        }else{
            return $count;
