...
use app\modules\eoffice_materialsys\models\TypeSearch;
...
class TypeController extends Controller
{
    ...
    public function actionCreate()
    {
        ...
        $data= MatsysMaterialType::find()->where(['material_type_id' => $_POST['type_id']])->exists();
        if ($data) {
            \Yii::$app->session->setFlash( 'warning', "มีข้อมูลในระบบแล้ว ไม่สามารถเพิ่มได้" );
            return $this->redirect("@web/eoffice_materialsys/type/index");
        } else {
            $model = new MatsysMaterialType();
            $model->material_type_id = $_POST['type_id'];

            $model->material_type_name = $_POST['type_name'];
